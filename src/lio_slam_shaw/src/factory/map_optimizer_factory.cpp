...
core::IMapOptimizer::SharedPtr MapOptimizerFactory::create(rclcpp::Node* node) {
    const std::string type =
        node->declare_parameter<std::string>("backend.map_optimizer_type", "gtsam");
    ...
    if (type == "gtsam") {
        ...
        params.isam2_relinearize_threshold = node->declare_parameter<double>(
            "backend.map_optimizer.isam2_relinearize_threshold", 0.1);
