...
std::shared_ptr<map_builder::IkdTreeLocalMapBuilder> LocalMapBuilderFactory::create(
    ...
    switch (type) {
        case LocalMapType::IKD_TREE:
            break;
    }
    map_builder::IkdTreeLocalMapBuilderParams params;
    params.ikd_delete_param = static_cast<float>(
        node->declare_parameter<double>("frontend.local_map.ikd_delete_param", 0.5));
    params.ikd_balance_param = static_cast<float>(
        node->declare_parameter<double>("frontend.local_map.ikd_balance_param", 0.6));
    params.ikd_downsample_size = static_cast<float>(
        node->declare_parameter<double>("frontend.local_map.ikd_downsample_size", 0.3));
    params.box_trim_half_length =
        node->declare_parameter<double>("frontend.local_map.box_trim_half_length", 50.0);
